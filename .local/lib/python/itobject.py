# TODO: put in right pythonX.Y or something
_install_dunders = lambda *names: lambda it: ([setattr(it, name, lambda self, *va, _clo_name_=name, **ka: (getattr(it, it.__fake__(self)).append((_clo_name_, va, ka)), self)[1]) for name in names], it)[1]
# @see https://docs.python.org/3/reference/datamodel.html#special-method-names
# XXX: cannot use `__new__`, `__init__` nor `__call__`; also `__repr__`, `__str__` and `__bytes__` basically useless...
...
class it:
    def __fake__(self): return '_ops_done_' + str(id(self))
    def __init__(self): setattr(it, it.__fake__(self), [])
    def __call__(self, the):
        niw = the
        for op, va, ka in getattr(it, it.__fake__(self)): niw = getattr(type(niw), op)(niw, *(v(the) if isinstance(v, it) else v for v in va), **dict((k, v(the) if isinstance(v, it) else v) for k,v in ka))
        return niw
